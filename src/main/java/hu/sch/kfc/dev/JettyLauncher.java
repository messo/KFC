 ...
 * 
 * Azért kellett ezt módosítani, hogy beolvassa a jetty-env.xml-et és a többi finomságot
 * ha kell, a verzió alapján lehet látni, hogy melyik Launcher lett átírva, ha verzió-frissítés
 * történik akkor célszerű egy újat leszedni és abban is megszerkeszteni, amit kell, lásd:
...
public class JettyLauncher extends ServletContainerLauncher {
        ...
        public void log(Request request, Response response) {
            ...
            if (status >= 500) {
                ...
                if ("/favicon.ico".equals(request.getRequestURI())
                    ...
                    /*
                     * We do not want to call the developer's attention to a 404 when requesting
                     * favicon.ico. This is a very common 404.
                     */
    ...
    protected static class JettyServletContainer extends ServletContainer {
        ...
        public JettyServletContainer(TreeLogger logger, Server server, WebAppContext wac,
    ...
    protected static final class WebAppContextWithReload extends WebAppContext {
        ...
        private class WebAppClassLoaderExtension extends WebAppClassLoader {
            ...
            private boolean addContainingClassPathEntry(String warnMessage, URL resource,
                    String resourceName) {
                TreeLogger.Type logLevel = (System.getProperty(PROPERTY_NOWARN_WEBAPP_CLASSPATH) == null) ? TreeLogger.WARN
                        : TreeLogger.DEBUG;
                ...
                branch = branch.branch(logLevel, "Adding classpath entry '" + classPathURL
                ...
                try {
                    ...
                    branch.log(TreeLogger.ERROR, "Failed add container URL: '" + classPathURL
                            + '\'', e);
                    return false;
                }
            }
        ...
        /**
         * Parent ClassLoader for the Jetty web app, which can only load JVM classes. We would just
         * use <code>null</code> for the parent ClassLoader except this makes Jetty unhappy.
         */
        ...
        /**
         * In the usual case of launching {@link com.google.gwt.dev.DevMode}, this will always by
         * the system app ClassLoader.
         */
    ...
    protected JettyServletContainer createServletContainer(TreeLogger logger, File appRootDir,
            Server server, WebAppContext wac, int localPort) {
